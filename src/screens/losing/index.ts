...
export class LosingScene extends Container<Text | Rect> {
	private static instance: LosingScene;
	...
	private constructor() {
		...
		const winText = new Text("YOU LOST", { font: "48px pixelmania", fill: "#fff" });
		winText.pos = { x: (800 - 490) / 2, y: 200 };
		this.add(winText);
	...
	static getInstance() {
		if (!LosingScene.instance) LosingScene.instance = new LosingScene();
		return LosingScene.instance;
	}
